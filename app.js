...
mongoose.connect("mongodb+srv://QaderiKabulTime:Qaderi@kabultime_12345@cluster0.yfxcz.mongodb.net/ShopDb?retryWrites=true&w=majority&appName=Cluster0");
const app = express();
exports.app = app;
      ...
      customer.find({}).sort({ name: 1 }).then((ct) => {
        stock.find({}).sort({ item: 1 }).then((it) => {
          currency.find({}).then((cr) => {
            res.render('order1', { bb: all, cit: ct, iit: it, cur: cr })
          })
        })
      })
    })
